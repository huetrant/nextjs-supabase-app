...
import { Customer, StoreData, Product, Variant, Order, OrderItem } from '@/types';
...
export default function EditOrderPage() {
  ...
  const fetchOrderData = async () => {
    try {
      ...
      if (orderData) {
        setSelectedCustomer(orderData.customer_id || '');
        ...
        // Fetch product and variant details for each order item
        ...
        if (orderItems) {
          for (const item of orderItems) {
            ...
            const { data: product, error: productError } = await supabase
              .from('product')
              .select('id, name, descriptions, link_image')
            ...
            if (item.variant_id) {
              const { data: variantData, error: variantError } = await supabase
                .from('variant')
                .select('id, Beverage_Option, calories, dietary_fibre_g, sugars_g, protein_g, caffeine_mg, price')
      ...
      <div className={`min-h-screen ${beverageColors.gradients.warm} p-6 space-y-6`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <TableBody>
                      {orderItems.map((item, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <VariantTableCell variant={item.variants} />
                          </TableCell>
                          <TableCell>
          ...
          <div className="flex justify-end space-x-4">
            ...
            <ActionButtons.Save
              type="submit"
              disabled={saving || !selectedCustomer || !selectedStore || orderItems.length === 0}
            >
              {saving ? <LoadingSpinner size="sm" /> : t('orders.updateOrder')}
            </ActionButtons.Save>
          </div>
        </form>
      </div>
